  ...
  //打开添加窗口
    function add(){
    	...
    	layer.open({
    	  ...
    	  shade: 0.8,
    	  area: [w+'px', h+'px'],
    	  content: 'basicgarden/class/add' //iframe的url
    	});
    }
  //打开编辑窗口
    function edit(gid){
    	...
    	layer.open({
    	  ...
    	  shade: 0.8,
    	  area: [w+'px', h+'px'],
    	  content: 'basicgarden/class/edit?cid='+gid //iframe的url
    	});
    }
    ...
    function del(ids){
    	...
    	$.post('basicgarden/class/delete',{ids:ids},function(result){
    		if(result.success){
    			layer.closeAll();
    			layer.msg('删除成功');
    			reload_data();
    ...
    function table_edit_update(id,field,value){
    	alert(11);
    	$.post('basicgarden/class/edit?'+field+'='+value,{id:id},function(result){
    		if(result.success){
    			layer.msg('修改成功');
    		}else{
    			layer.closeAll();
    ...
    <div class="box">
		...
		<div class="layui-btn-group">
	 	<button onclick="add()" class="layui-btn   layui-btn-sm"><i class="layui-icon">&#xe654;</i>增加</button>
	  	<button class="layui-btn layui-btn-sm" onclick="delSelected2()"><i class="layui-icon">&#xe640;</i>删除选中项</button>
...
layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel',
		...
		, element = layui.element; //元素操作
		  table.render({
            elem: '#table'
            ,url: '<%=request.getContextPath()%>/basicgarden/list_class'
            ,height: 'full-70'
            ,cellMinWidth: 70
            ,cols: [[ //表头
            {type:'checkbox',width: 30,align:'center'}
            ,{field: 'cname', title: '班级',align:'center'} 
            ,{field: 'gradeid', title: '年级',align:'center'} 
            ,{field: 'cid', title: '编号',minWidth: 150,align:'center'}
