g	var employeeEntitysArray = new Array();//保存选择的商品数组
	function cancel1(){
	...
	function submitDialog(){
		var ckb_chkIds = $.pdialog.getCurrent().find("input:[name=employeeEntitychkIds]");// 
		alert(ckb_chkIds);
		var checkedCount = 0;
		...
		//window.returnValue=employeeEntitysArray;
		//window.close();
		this.$("div.dialog").hide();
	    this.$("div.shadow").remove();
	    _dialogMultiemployeeEntityReturnDo(employeeEntitysArray,this);//调用提交页面返回操作
	}
	// 动态数组
	...
	function selectedUserChk(obj,id,number,name,k){
		if (k>0) { employeeEntitysArray = new Array(); }
		...
		userObj.number=number;
	 	userObj.name=name;
	 	employeeEntitysArray = employeeEntitysArray.append(userObj);
	    }else{
	    	employeeEntitysArray = employeeEntitysArray.remove(userObj); 
	   }
    }
