	var usersArray = new Array();//保存选择的商品数组
	function cancel1(){
	...
	function submit1(){
		var ckb_chkIds = document.getElementsByName("chkIdsUserMulSel");
		var checkedCount = 0;
		...
		//window.returnValue=usersArray;
		//window.close();
		this.$("div.dialog").hide();
	    this.$("div.shadow").remove();
	    _dialogMultiUserReturnDo(usersArray,this);//调用提交页面返回操作
		
	}
	
	
	function selectedUserChk(obj,id,loginId,name,k){ 
		if (k>0) { usersArray = new Array(); }
        ...
        {
	 	userObj.id=id;
		userObj.loginId=loginId;
	 	userObj.name=name;
	    usersArray = usersArray.append(userObj);
	    }else{
	    	usersArray = usersArray.remove(userObj); 
	   }
    }
	function aa(id){
