<%@ page language="java" import="java.util.*,cn.com.hinova.ruvs.common.*" pageEncoding="UTF-8"%>

<%@ include file="/WEB-INF/jsp/includes/header.page.jsp"%>
<%
	request.setAttribute("superAdminLoginName",Config.superAdminLoginName);
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false" style="height:50px;overflow: hidden;">
		<div id="search_area" >
			<span><input type="text" class="easyui-textbox" name="like:String:location" data-options="prompt:'安装位置'"></span>
		...
		<table id="_data_grid" data-options="method:'get',border:false,fit:true,fitColumns:true">
			<thead>
				<tr>
					...
					<th data-options="field:'location',sortable:true" style="width:90px">安装位置</th>
					<%--<th data-options="field:'ip',sortable:true" style="width:90px">IP地址</th>--%>
					<%--<th data-options="field:'mac',sortable:true" style="width:90px">MAC地址</th>--%>
					<th data-options="field:'sn',sortable:true" style="width:90px">SN号</th>
					<th data-options="field:'status',sortable:true, formatter: _status" style="width:90px" >链接状态</th>
...
">加载中...</div>
    ...
    <script type="text/javascript">
        ...
        _data_grid.datagrid({
            ...
            sortName:"editTime",
            sortOrder:"desc"
            <c:if test="${user.loginName eq superAdminLoginName }">
            ,toolbar : [ {
                text : "添加",
                iconCls : 'icon-add',
                handler : function() {
                ...
                handler : function() {
                    ...
                    if (selected_rows.length == 0) {
                        ...
                        top.alert("每次只能同步一台设备。");
                        return;
                    }
                    var deviceIp=selected_rows[0]["ip"];

                    $("#synInfo").dialog("open");
                    $.get("config/patriarch/synPatriarchByDevice.do?deviceIp="+deviceIp,function(data){

						if(data.code==200){

                            $("#synInfo").html(data.msg+"<br/>并且同步了所有家长的模板信息。").css("padding","40px");
						}else{
                            $("#synInfo").text("操作失败！");
						}
					},"json");
